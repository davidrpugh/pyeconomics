...
!   First record where they come from
	...
	cutvecttemp(icut)=Cutoff(iz,izp,ieps,iepsp)
	zindtemp(icut)=iz
	zpindtemp(icut)=izp
	epsindtemp(icut)=ieps
	epspindtemp(icut)=iepsp !not really epsp
	icut=icut+1
...
!   record the values of z,zp and eps that they came from
	...
	do icut=1,ncut
	  ...
	  epspind(icut)=epspindtemp(indcut(icut))
...
!   probability and evaluating points
	...
	if (evalpoint(iinterv)<Cutoff(iz,izp,ieps,1)) then !e
	gbar(iinterv,iz,izp,ieps,4)=1.0D0
	else
	gbar(iinterv,iz,izp,ieps,4)=0.0D0
	end if

	if (evalpoint(iinterv)>Cutoff(iz,izp,ieps,1) .and. evalpoint(iinterv)<Cutoff(iz,izp,ieps,2)) then !s
	gbar(iinterv,iz,izp,ieps,3)=1.0D0
	else
	gbar(iinterv,iz,izp,ieps,3)=0.0D0
	end if

	if (evalpoint(iinterv)>Cutoff(iz,izp,ieps,2) .and. evalpoint(iinterv)<Cutoff(iz,izp,ieps,3)) then !f
	gbar(iinterv,iz,izp,ieps,2)=1.0D0
	else
	gbar(iinterv,iz,izp,ieps,2)=0.0D0
	end if

	if (evalpoint(iinterv)>Cutoff(iz,izp,ieps,3)) then !s
	gbar(iinterv,iz,izp,ieps,1)=1.0D0
	else
	gbar(iinterv,iz,izp,ieps,1)=0.0D0
	end if
	...
	open(1, file='Output\cutvect.txt', status='unknown')
	write(1, '(F10.6)') cutvect
