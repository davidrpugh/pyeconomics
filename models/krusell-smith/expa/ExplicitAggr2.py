...
# To get more nodes close to the constraint, use eqmudistant nodes in log-space
kmin   = 0.0
kmax   = 200.0

kp = kmin + np.logspace(0, np.log(1 + kmax), Nk, base=np.exp(1)) - 1 
...
Ku = np.linspace(Kumin, Kumax, NKu).reshape(NKu, 1)

##### Grid for aggregate capital stock of employed #####

# Specify the number of nodes for approximating Ke
NKe    = 12
...
Ke = np.linspace(Kemin, Kemax, NKe).reshape(NKe, 1)
...
kpp1 = np.tile((1 - delta) * kp[:,np.newaxis], reps=(2, 2, NKu, 1, NKe))

# Individual policy (conditional on being unemployed?) Why 0.3??
kpp2 = np.tile(0.3 * (1 - delta) * kp[:,np.newaxis], reps=(2, 2, NKu, 1, NKe))

# not sure which is right way to do the grid. This version, or...
...
# ...this version!
Kemat, kmat, Kumat = np.meshgrid(Ke, kp, Ku, indexing='xy')
...
while ConvCrit > 1e-6:
    s = s+1
k = np.zeros((2,2,NKe,kp.shape[0],NKu))
    
for i in xrange(2): # pages
    for j in xrange(NKe): # rows
        for l in xrange(NKu): # columns
            ...
            RHS = np.hstack((beta * Rp[0] * (Rp[0] * kp + Wp[0] - kpp1[i,0,l,:,j].reshape((kp.shape[0],1)))**(-sigma), 
                             beta * Rp[1] * (Rp[1] * kp + mu * Wp[1] - kpp2[i,0,l,:,j].reshape((kp.shape[0],1)))**(-sigma),
                             beta * Rp[2] * (Rp[2] * kp + Wp[2] - kpp1[i,1,l,:,j].reshape((kp.shape[0],1)))**(-sigma),
                             beta * Rp[3] * (Rp[3] * kp + mu * Wp[3] - kpp2[i,1,l,:,j].reshape((kp.shape[0],1)))**(-sigma)))
            ...
            k[i,1,l,:,j] = (C2 - mu * (1 - alpha) * Z[i] * (K / (h * (1 - Unemp[i])))**(alpha) + kp.flatten()) / \
                           ((1 + alpha * Z[i] * (K / (h * (1 - Unemp[i])))**(alpha - 1) - delta))
...
KpeFit = interp2(KuMat2,KeMat2,Kpe(:,:,ZSim(i)),Kmump(i),KeImp(i));
KpuFit = interp2(KuMat2,KeMat2,Kpu(:,:,ZSim(i)),Kmump(i),KeImp(i));
...
K = (1-Unemp(ZSim(i)))*KeImp(i)+Unemp(ZSim(i))*Kmump(i);
r = 1+alpha*ZZ(ZSim(i))*(K/(h*(1-Unemp(ZSim(i)))))^(alpha-1)-delta;
w = (1-alpha)*ZZ(ZSim(i))*(K/(h*(1-Unemp(ZSim(i)))))^(alpha);
Cind(i,1) = r*Kind(i)+(2-ind_switch(i))*h*w*(1-tau(ZSim(i)))+(ind_switch(i)-1)*w*mu-Kind(i+1);
Rvec(i) = r;
Wvec(i) = w;
...
KImp = (1-Unemp(ZSim)).*KeImp+Unemp(ZSim).*Kmump;
...
disp('Max error Ku (%)')
100*max(abs(log(KuSim)-log(Kmump)))
disp('Mean error Ke (%)')
100*mean(abs(log(KeSim)-log(KeImp)))
disp('Mean error Ku (%)')
100*mean(abs(log(KuSim)-log(Kmump)))
disp('R-Square K')
1-var(KImp-KFit)/var(KImp)
disp('R-Square Ke')
1-var(KeImp-KeFit)/var(KeImp)
disp('R-Square Ku')
1-var(Kmump-KuFit)/var(Kmump)
"""
