...
def naiveUnivariateSplineBellman(v, u, discount, Gamma, order):
    ...
    for i, k in enumerate(grid):
        ...
        pol = maximize(tmp_v, Gamma(k, 0.0))
...
tmp_v = lambda kplus: (1 - beta) * u(Gk[i], kplus) + beta * initial_v(kplus)
neg_tmp_v = lambda kplus: -tmp_v(kplus)

plt.plot(plot_grid, tmp_v(plot_grid), label='Linear')
plt.xlabel('Capital, $k$', fontsize=15)
plt.ylabel('$TV_0(k)$', rotation='horizontal', fontsize=15)
plt.title('T preserves monotonicity with linear Spline?', fontsize=15)
plt.legend(loc='best', frameon=False)
plt.show()
